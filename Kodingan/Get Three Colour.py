...
for i, (scaled, original) in enumerate(zip(scaled_colors, dominant_colors)):
    r, g, b = scaled
    ax.scatter(r, g, b, color=f"#{original[0]:02x}{original[1]:02x}{original[2]:02x}", s=150, label=f"W{i}")
    ax.text(r, g, b, f"W{i}", fontsize=10, color="black")

# Atur label dan judul grafik
ax.set_xlabel("Red (scaled)")
ax.set_ylabel("Green (scaled)")
ax.set_zlabel("Blue (scaled)")
ax.set_title("Perbandingan Warna Dominan dengan Perbedaan yang Ditingkatkan pada Pasir Gelap")
ax.legend()
